...
5. 录音数据保存在芯片RAM，**断电丢失，不会保存到Flash**。
...
## 常见故障排查
1. 一直自动循环录音播放：GPIO引脚冲突，同一个GPIO不能同时分配给I2S和按键。
2. 完全没有声音：
   - 检查MAX98357的SD引脚，不能接GND；
   - 检查三根I2S信号线（BCLK、LRC、DIN）是否接反、虚焊；
   - 确认所有模块GND共地；
   - 喇叭接在OUT+、OUT-，不能一端接GND。
3. 录音音量偏小：MAX98357的GAIN引脚接到3.3V提升增益。
4. 采样点为0：麦克风接线错误，INMP441供电异常。
